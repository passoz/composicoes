# Stem: Sopro/Metal — Samba & Pagode Brasileiro

> **Kit fixo:** Ver `identidade-da-banda.md` para modelos e timbres específicos (trompete Bach Stradivarius 37, trombone King 3B, sax tenor Selmer Mark VI). Metais de samba — com presença e calor, não com brutalidade de rock.

## Identificação Sonora

### Instrumentos Típicos
- **2 Trompetes**: uníssono, registro médio-agudo (Dó4 a Sol5)
- **1 Trombone**: voz inferior, registro médio (Fá3 a Dó4)
- **1 Saxofone tenor**: contracanto, registro médio-grave (Si2 a Sol4)

### Características Timbricas Fundamentais
Os metais no samba/pagode **não são como big band americana**. A diferença essencial:

| Samba/Pagode | Big Band Americana |
|---|---|
| Staccato curto e definido, notas com ataque | Notas longas, sostenuto |
| Uníssono rítmico (todos na mesma nota, oitavas diferentes) | Harmonias em 4-5 partes, spreads |
| Frases curtas de 2-4 compassos em loop | Desenvolvimento melódico longo |
| Arranjo em bloco —似 "parede de som" | Arranjo aberto, texturas variadas |
| Presença e calor, sem brutalidade | Som limpo e blendado |

### Frequências-Chave para Separação
| Instrumento | Faixa Dominante | Característica |
|---|---|---|
| Trompete (fundamental) | 300Hz-1.2kHz | Agudo, brilhante, cortante |
| Trompete (brilho) | 2-8kHz | Ataque, brightness, definition |
| Trombone (fundamental) | 150-500Hz | Médio, encorpado |
| Trombone (brilho) | 1-4kHz | Ataque, glissando audível |
| Sax tenor (fundamental) | 100-600Hz | Médio-quente, breathy |
| Sax tenor (brilho) | 1-5kHz | Ataque, vibrato, "chorado" |

### Padrão Rítmico Característico
Os metais reforçam o contratempo em uníssono com o cavaquinho e violão:

```
Cavaquinho: . x . x . x . x
Metais:     . x . x . x . x   (mesmo ataque, 8va acima)
```

- Acentuação **sempre no contratempo** (2ª semicolcheia de cada tempo)
- Staccato curto e definido: notas com ataque, mas sem brutalidade — é samba, é pra cortar a mistura, não pra mata-la
- Repetição de riffs curtos (2-4 compassos), não desenvolvimento temático
- Arranjos **em bloco**, não abertos

### Trombone — Papel Especial
O trombone no samba não é voz de fundo — é **melódico**, cantando o contracanto da melodia principal. Glissandos frequentes e característicos.

### Sax Tenor — Cor Expressiva
O saxofone tenor no samba pode usar vibrato intenso e lábio tenso em notas longas, produzindo um som "chorado" que é uma cor expressiva característica. Não é uma "imitação de cuíca" literal, mas compartilha a qualidade vocal e expressiva desse instrumento.

### Quando os Metais Aparecem
- **Não tocam o tempo todo** — aparecem em refrões, pontes, clímax
- **Entram em bloco** — não gradualmente, todos de uma vez
- **Saem em silêncio** — sem fade, corta seco
- **Nos versos**, frequentemente só o sax fica (fazendo contracanto ou sustentando notas longas)

### Armadilhas Comuns na Separação
1. **Trompete vs cavaquinho**: Compartilham a faixa de ~1-4kHz em timbre e padrão rítmico. O trompete tem brilho mais metálico e harmônicos mais intensos; o cavaquinho tem timbre de corda dedada com mais "ruído" de ataque.
2. **Trombone vs tantã/rePique**: O trombone em registro médio (~150-500Hz) pode se sobrepor a percussão de mão. O trombone tem pitch definido e frases melódicas; a percussão tem padrões mais repetitivos e menos melódicos.
3. **Sax tenor vs baixo**: O sax em registro grave (~100-300Hz) pode se sobrepor ao baixo. O sax tem timbre breathy e vibrato; o baixo tem ataque percussivo distinto e movimento cromático característico.
4. **Staccato dos metais vs caixa**: O staccato definido dos trompetes pode ter transientes parecidos com ghost notes de caixa. A diferença: os metais tocam em frases (não contínuo) e sempre em uníssono.
5. **Growl de trompete vs vocal**: O growl pode ser confundido com vocais roucos. O growl tem frequência fundamental estável com harmônicos irregulares; a voz tem formantes variáveis.